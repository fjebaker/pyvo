...
class TestWebProfile(BaseTestStandardProfile):
    ...
    def test_web_profile(self):
        ...
        with get_readable_fileobj(
            f"http://127.0.0.1:{self.hub._web_port}/crossdomain.xml"
        ) as f:
            assert f.read() == CROSS_DOMAIN

        with get_readable_fileobj(
            f"http://127.0.0.1:{self.hub._web_port}/clientaccesspolicy.xml"
        ) as f:
            assert f.read() == CLIENT_ACCESS_POLICY

        # Check headers

        req = Request(f"http://127.0.0.1:{self.hub._web_port}/crossdomain.xml")
